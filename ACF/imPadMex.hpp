...
template <class T>
void imPad(T* A, T* B, int h, int w, int d, int pt, int pb,
...
#define PAD(XL, XM, XR, YT, YM, YB)                     \
    for (x = 0; x < pl; x++)                            \
        for (y = 0; y < pt; y++)                        \
            B[x * hb + y] = A[(XL + cl) * h + YT + ct]; \
    for (x = 0; x < pl; x++)                            \
        for (y = pt; y < h1; y++)                       \
            B[x * hb + y] = A[(XL + cl) * h + YM + ct]; \
    for (x = 0; x < pl; x++)                            \
        for (y = h1; y < hb; y++)                       \
            B[x * hb + y] = A[(XL + cl) * h + YB - cb]; \
    for (x = pl; x < w1; x++)                           \
        for (y = 0; y < pt; y++)                        \
            B[x * hb + y] = A[(XM + cl) * h + YT + ct]; \
    for (x = pl; x < w1; x++)                           \
        for (y = h1; y < hb; y++)                       \
            B[x * hb + y] = A[(XM + cl) * h + YB - cb]; \
    for (x = w1; x < wb; x++)                           \
        for (y = 0; y < pt; y++)                        \
            B[x * hb + y] = A[(XR - cr) * h + YT + ct]; \
    for (x = w1; x < wb; x++)                           \
        for (y = pt; y < h1; y++)                       \
            B[x * hb + y] = A[(XR - cr) * h + YM + ct]; \
    for (x = w1; x < wb; x++)                           \
        for (y = h1; y < hb; y++)                       \
            B[x * hb + y] = A[(XR - cr) * h + YB - cb];
