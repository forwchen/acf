...
void convTriY(float* I, float* O, int h, int r, int s)
{
    r++;
    float t, u;
    int j, r0 = r - 1, r1 = r + 1, r2 = 2 * h - r, h0 = r + 1, h1 = h - r + 1, h2 = h;
    u = t = I[0];
    for (j = 1; j < r; j++)
        u += t += I[j];
    u = 2 * u - t;
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    ...
    if (s < 1)
        mexErrMsgTxt("Invalid sampling value s");
    if (r < 0)
        mexErrMsgTxt("Invalid radius r");
